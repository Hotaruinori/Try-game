...
public class BabyA { // 類別名稱改為 Boss
    ...
    public BabyA() {
        ...
        // 初始化用於繪製的 Sprite
        babyADisplaySprite = new Sprite(monsterIdleAnimation.getKeyFrame(0)); // 初始使用待機動畫的第一幀
    ...
    public void update(float deltaTime) {
        ...
        if (distance > (monsterWidth / 2 + character.getWidth() / 2) * 0.8f) { // 當距離大於 Boss 和玩家半徑和的0.8倍時才追蹤，避免過於靠近
            ...
            // 根據移動方向設定 Sprite 的翻轉
            if (velocityX < 0) { // 向左移動
                if (!babyADisplaySprite.isFlipX()) {
                    babyADisplaySprite.flip(true, false);
                }
        ...
        if (BabyA_Rectangle().overlaps(character.getCharacterRectangle())) {
    ...
    public void render(SpriteBatch batch) {
        ...
        babyADisplaySprite.setRegion(currentFrame);
        babyADisplaySprite.draw(batch);
    }
    ...
    // Boss 的碰撞矩形
    public Rectangle BabyA_Rectangle() {
        return babyADisplaySprite.getBoundingRectangle();
    }
...
//        System.out.println("BabyA 受到 " + damageAmount + " 點傷害！當前 HP: " + currentHealth);

        if (currentHealth <= 0) {
            currentHealth = 0;
            isAlive = false;
            System.out.println("BabyA 已被擊敗！");
    ...
    // --- 位置獲取和設定方法 ---
    public float getX() {
        return babyADisplaySprite.getX(); // 直接從 sprite 獲取位置
    }

    public float getY() {
        return babyADisplaySprite.getY(); // 直接從 sprite 獲取位置
    }

    public Vector2 getCenterPosition() {
        return new Vector2(babyADisplaySprite.getX() + babyADisplaySprite.getWidth() / 2,
            babyADisplaySprite.getY() + babyADisplaySprite.getHeight() / 2);
    }

    public void setX(float x) {
        this.posX = x; // 更新內部坐標 (可選，但保持一致性較好)
        babyADisplaySprite.setX(x);
    }

    public void setY(float y) {
        this.posY = y; // 更新內部坐標
        babyADisplaySprite.setY(y);
    }
